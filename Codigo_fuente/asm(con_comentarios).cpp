...
#define MAX_LINE_COUNT 1000 /* max 1000 lines allowed */            // M�ximo numero de lineas del programa, incluye lineas con comentarios.
...
#define instruction_count 31/* total instruction set */             // Numero m�ximo de instrucciones
...
char linebuf[200];                                                 // Linea del programa analizada por el Parser
int line_count = 0;
int constant_count = 0;
int reg_count = 0;
unsigned program_word[PROGRAM_COUNT]; /* program word array */     // Matria para almacenar el codigo m�guina
...
typedef struct opcode {                                            //estructura con los campos de la instrucci�n
...
int error = 0;
/*====================================== */
void free_mem(void)                                                       // libera la estructura que contiene los campos de la instruccion
{
	int i;
...
/*====================================== */
void init_program_word(void)                                               // libera la matriz del programa
{
	int i;
...
/*====================================== */
/* convert hex string to int, return -1 if not valid */                    // Convierte cadena en hex a entero.
int htoi(char *s)
{
	int i, l, n = 0;
	char *p;
...
/*====================================== */
/* Only S0 - S7 are valid */                                               // Comprueba la sintaxis de los registros.
...
/*====================================== */
void insert_instruction(char *s, int p)                                    // crea codigo m�quina: campo de instrucciones
{
	int i, l;
	unsigned n = 0;
...
/*====================================== */
void insert_sXX(int c, int p)                                              // crea codigo m�quina: campo 1er registro                  
{
	program_word[p] = program_word[p] | (unsigned) (c << 8);
}

/*====================================== */
void insert_sYY(int c, int p)                                              // crea codigo m�quina: campo 2er registro 
{
	program_word[p] = program_word[p] | (unsigned) (c << 5);
}

/*====================================== */
void insert_constant(int c, int p)                                         // crea codigo m�quina: campo Inmediato
{
	program_word[p] = program_word[p] | (unsigned) (c);
}

/*====================================== */
void insert_flag(int c, int p)                                             // crea codigo m�quina: campo Flag
{
	program_word[p] = program_word[p] | (unsigned) (c << 8);
}

/*====================================== */
int decode_flag(char *s)                                                   // decodifica Flag
{
...
/*====================================== */
int find_constant(char *s)                                                 // busca Inmediato
{
	int i;
...
/*====================================== */
int find_label(char *s)                                                    // busca Etiqueta        
{
	int i;
...
/*====================================== */
int find_namereg(char *s)                                                  // busca Nombre asignado a registro
{
	int i;
...
/*====================================== */
int parse_linebuf(void)                                                    // analizado de lineas del programa
{
	char *ptr;
	char seps[]   = " :;,\t\n";
    char *token;
...
/*====================================== */
/* syntax test and assign addresses */                                     // comprobaci�n de sintaxis de los campos de la instruccion
void test_instructions(void)
{
	...
	for(i = 0; i < line_count; i++){                                      // para todas las lineas del programa
		if(op[i].instruction != NULL){                                    // si la linea contiene codigo de operaci�n comprueba si conincide con las definidas en el juego de instrucciones
			...
			switch (j)                                                    // comprueba a que clase de codigo de operaci�n pertenece
			{
				case 0: /* JUMP */
				case 1: /* CALL */                                        // Para el caso CALL comprueba que si tiene operando2, el operando1 debe ser un Flag (Z,NZ,C,NC)
					...
					} else if(op[i].op1 == NULL){                           // Para el caso CALL comprueba que tiene operando1 (todos instrucciones CALL tienen primer operando)
				...
				case 2: /* RETURN */
					...
					break;                                                  // Para instrucciones aritmetica l�gicas comprueba que tiene operando1 y operando2.
				case 3: /* LOAD */
				case 4: /* AND */
				case 5: /* OR */
				case 6: /* XOR */
				...
				case 25: /* DISABLE */
					...
					break;
				case 26: /* CONSTANT */                                                 // ojo no es una instruccion es una directiva
					if((op[i].op1 == NULL) || (op[i].op2 == NULL)){
					...
					break;
				case 27: /* NAMEREG */                                                   // ojo no es una instruccion es una directiva                 
					if((op[i].op1 == NULL) || (op[i].op2 == NULL)){
					...
					break;
				case 28: /* ADDRESS */                                                   // ojo no es una instruccion es una directiva
			...
			op[i].address = address;                                                      // si la instrucci�n analizada es una instruccion (no directiva) incrementa la direcci�n de memoria donde se almacena.
...
void write_program_word(void)                                                             // analizador de instrucciones y escritura de c�digo m�quina.
{
	int i, j, reg_n;
	char *kptr, *sptr;
		...
		if(op[i].instruction != NULL){                                                   // si la linea contiene codigo de operaci�n comprueba si conincide con las definidas en el juego de instrucciones
			...
			switch (j)                                                                   // comprueba a que clase de codigo de operaci�n pertenece
			{
				...
				case 1: /* CALL */                                                       // para el caso de CALL o JUMP
					if(j == 0)
						kptr = jump_id;                                                  // comprueba el codigo de operacion si es JUMP o CALL (kptr: instrucci�n cuyo operando es una constante)
					else
						kptr = call_id;
					insert_instruction(kptr, op[i].address);                             // inserta codigo maquina del campo de operaci�n.
					if(op[i].op2 == NULL){                                               // comprueba si no hay operando2 --> solo 2 instrucciones: CALL aa; JUMP aa
						if((reg_n = find_label(op[i].op1)) != -1){                                    // revisa si es etiqueta, constante, si es hexadecimal, o si es un operando no valido.
							insert_constant(reg_n, op[i].address);                                    // inserta codigo maquina de direcci�n 
						...
						if(op[i].op1 != NULL){                                            // comprueba que hay operando1 -->  instrucciones con Flag: CALL NZ,aa ; JUMP Z,aa, etc
							reg_n = decode_flag(op[i].op1);
							insert_flag(reg_n, op[i].address);                            // revisa Flag e inserca codigo maquina del Flag
						}
						if((reg_n = find_label(op[i].op2)) != -1){                        // inserta codigo maquina de direcci�n (operando 2)
							insert_constant(reg_n, op[i].address);
				...
				case 2: /* RETURN */
					...
					break;
				case 3: /* LOAD */                                                                // para el caso de instrucciones aritmetico logicas.
				case 4: /* AND */
				case 5: /* OR */
				case 6: /* XOR */
				case 7: /* ADD */
				...
				case 9: /* SUB */
				case 10: /* SUBCY */
					if(j == 3){ kptr = load_k_to_x_id; sptr = load_y_to_x_id;}                    // determina el codigo m�quina del codigo de operaci�n
					if(j == 4){ kptr = and_k_to_x_id; sptr = and_y_to_x_id;}                      // (sptr: instrucci�n cuyo 2do operando es un registro)
					if(j == 5){ kptr = or_k_to_x_id; sptr = or_y_to_x_id;}                        // (kptr: instrucci�n cuyo 2do operando es una constante)
					...
					if((reg_n = find_namereg(op[i].op1)) != -1)                                   // comprueba si el 1er operando es un namereg o un registro, sino error.
						insert_sXX(reg_n, op[i].address);                                         // inserta codigo maquina del primer operando
					...
					if((reg_n = find_constant(op[i].op2)) != -1){                                 // comprueba si el 2do operando es un namereg, un registro, una constante o un hexadecimal, sino error.
						insert_constant(reg_n, op[i].address);                                    // inserta codigo maquina del segundo operando
				...
				case 25: /* DISABLE */
					...
					if(j == 24) /* ENABLE */
						program_word[op[i].address]++;
					break;
				case 26: /* CONSTANT */                                                  // son directivas no tiene codigo maquina asociado
				case 27: /* NAMEREG */
				case 28: /* ADDRESS */
					break;
...
/*====================================== */
void write_fmt(void)                                                                     // imprime fichero de salida en formato 'fmt'                                                      
{
	char *ptr;
	int i;
...
/*====================================== */
void write_log(void)                                                                     // imprime fichero de salida en formato 'log'
{
	int i;
...
/*====================================== */
/* write program word in hex format */                                                   // imprime fichero de salida en formato 'bin'
void write_bin(void)
{
	int i;
	char *ptr;
...
/*====================================== */
/* write intel mcs file for programming */                                               // imprime fichero de salida en formato 'mcs'
void write_mcs(void)
{
	unsigned int i,j,k;
	char *ptr;
...
/*====================================== */
/* write vhdl module for simulation */                                                   // imprime fichero de salida en formato 'vhdl'
void write_vhd(void)
{
	int i, j;
	char *ptr;
...
int main(int argc, char **argv)                                                                 // MAIN
{
    ...
    if(argc != 2){
		printf("\nCommand line syntax:\n\nasm file.asm\n");                                     // FUNCIONES APERTURA DE FICHEROS 
		exit(1);
	}
	...
	*ptr = '\0';
	...
	while ( fgets(linebuf, 128, ifp) != NULL ) {                                                // FUNCION LECTURA DE FICHERO FILE.ASM
	...
	init_program_word();                                                                        // BORRA EL CONTENIDO DE LA MATRIZ QUE CONTIENE CODIGO MAQUINA.
	printf("Testing instructions...\n");
	fprintf(ofp,"Testing instructions...\n");
	test_instructions();                                                                        // ANALIZA DE LA SISTAXIS DEL CODIGO ENSAMBLADOR.
	if(error > 0){
		printf("Program aborted...error count %d\n", error);
		fprintf(ofp,"Program aborted...error count %d\n", error);
		error_out();
	}
	write_program_word();                                                                       // ANALIZA INSTRUCCIONES Y ESCRIBE CODIGO MAQUINA.
	if(error > 0){
		printf("Program aborted...error count %d\n", error);
		fprintf(ofp,"Program aborted...error count %d\n", error);
		error_out();
	...
	free_mem();                                                                               // LIBERA ESTRUCTURA CON CAMPOS DE OPERACI�N
